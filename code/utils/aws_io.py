...
def aws_s3_sync(source_directory, destination_bucket):
    try:
        ...
        logger.info(f'Successfully synced {source_directory} to {destination_bucket}')
    except subprocess.CalledProcessError as error:
        logger.info(f'Failed to sync {source_directory} to {destination_bucket}. The error message is {error.output}')
